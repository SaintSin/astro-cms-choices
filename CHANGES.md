...
- `detect-cms.ts` had `main().catch(...)` running unconditionally at module scope, with no entry-point guard. Importing its exports for reuse therefore also re-ran the _entire_ scan pipeline as a side effect of loading the module. A `--limit 2` smoke test of the new script silently kicked off a real (if tiny) `pnpm detect` run in the background
- That run wrote a legitimate-looking `scans` row (and, oddly, a duplicate — scans #147 and #148, same 2 sites, likely a double module-evaluation quirk from the mixed `import type` + value import from the same `.ts` file) that became "the latest scan" — the exact same corruption shape as the 2026-08-15 incident, just via a new trigger. Worse this time: the _output write_ wasn't additive — `cms-results.json` (2,925 sites) was fully overwritten down to the 2 sites that tiny run touched
...
- `cms-results.json` couldn't be cleanly reconstructed from the DB alone — `evidence`, `framework`, `categories`, and `dateAdded` were never persisted there, only in the YAML source + live fingerprinting (see below, fixed). Kicked off a full `pnpm detect` re-run instead of a lossy partial rebuild. The currently-_deployed_ site was unaffected throughout — `cms-results.json` is gitignored and only gets baked in at `pnpm build`/deploy time, so this was local-only damage, but it would have shipped on the next deploy if left unfixed
...
- Deliberately did _not_ add `framework` — it's already effectively broken (`effectiveHit?.framework` only ever populates when there's _also_ a separate CMS hit alongside the framework match, so it's almost always `null` in practice today). Not worth persisting a broken field; revisit once that detection logic itself is fixed

### `recheck-blocked.ts`/`merge-recheck-results.ts`: cleared most of the `Blocked` bucket

- Ran the full `Blocked` sweep (122 sites) through real headless Chromium. 74 changed on the first pass, but the dry-run before merging caught a real bug: `recheckSite()` called `fingerprint()` directly, skipping the same demotion rule `processSite()` normally applies — a `"Blocked"`/`"full-site"`/`"parked"` label sitting on top of confirmed Astro content is supposed to demote to `"Unknown"`. Several sites were merging as `"Blocked (astroDetected: true)"`, which is contradictory. Fixed before anything was applied
- Built `scripts/merge-recheck-results.ts` to apply a recheck report's `changed` entries onto `cms-results.json` and the DB's current latest scan without a full re-scan — dry-run by default, recomputes the scan's aggregate counts after writing since they'd otherwise go stale
- Re-ran with the fix: 73 reclassified. 63 turned out to be genuine Astro false negatives (several with a specific CMS on top — Sanity, Storyblok, Contentful, Wix), 1 confirmed non-Astro (`cointracker.io`, Sanity-backed), 22 real errors (TLS/SSL failures, correctly left alone), 27 genuine bot walls even a headless real browser can't pass. Applied
- `Blocked` count: 122 → 49 after the headless pass

### `--headed` mode for the sites a headless browser still can't pass

- Added `--headed`/`--only-still-blocked` to `recheck-blocked.ts` — opens a real, visible Chromium window one site at a time and waits for a human to solve whatever challenge appears, then press Enter to capture
- Found a footgun while testing: any bounded run (`--headed` or `--limit`) was overwriting `blocked-recheck-report.json`, the same file `--only-still-blocked` reads its target list from — a quick test run could silently destroy the reference a real run depends on. Bounded runs now write to `blocked-recheck-report.partial.json` instead (gitignored); `merge-recheck-results.ts` gained `--report` to target it
- Manually worked through the 27-site bot-wall bucket. Found the practical ceiling of this approach: Cloudflare's stronger challenge tiers detect the CDP automation layer itself (`navigator.webdriver`, missing plugins, CDP-specific artifacts), not "is this headless" — so a few sites loop the challenge forever even in a visible, human-watched window, because the automation signal never clears regardless of who's solving it. Not a bug to keep chasing; a real limitation of browser-automation-based checking
- Found 2 new parked-registrar patterns along the way, neither caught by existing rules: **HugeDomains** (title skips the word "domain" entirely — `"FreshCleanLiving.com is for sale | HugeDomains"` — so even the loosened generic parked-title fallback misses it) and **ExpiredDomains** (its own marketplace page Cloudflare-blocks a plain fetch, so the hostname match on the redirect destination is what actually catches it, not HTML content). Added dedicated detectors for both
- Net result across the full session: `Blocked` count 122 → 41. One site (`freshcleanliving.com`) remains stuck in the Cloudflare automation-detection loop even after the new detector existed — genuinely resistant, left as `Blocked`

## 2026-08-25

### Three showcase PRs, and a fork-drift bug in `make-removal-prs.mjs`

- [#2644](https://github.com/withastro/astro.build/pull/2644): removed `aidailynews.io` — domain repurposed _in place_ (same-origin 200, no HTTP redirect) to serve WordPress-built Vietnamese gambling content. The existing cross-host "Forwarded" detection can't catch this shape at all since there's no redirect to follow; only caught by reading the actual page content. Logged as a new pattern in `REVIEW-NOTES.md`
...
- A Starlight project lead pushed back on Discord: fair point that a single-site "real users saw it get faster" claim reads as more general than it is. Investigating further to properly caveat it uncovered a bigger problem — `github.github.com` is a shared GitHub Pages origin, and CrUX only reports at the origin level, not per page. Every CrUX number in the post was the combined real-user experience across _every_ project hosted there, not `/gh-stack/` specifically
- Pulled the full weekly trend via CrUX's History API (`records:queryHistoryRecord` — a different endpoint to the `records:queryRecord` our regular `crux.mjs` uses, and one it doesn't currently query) to check: origin-wide mobile INP peaks at 233ms on 2026-07-18, two weeks _before_ the migration, then falls to 147ms by 2026-08-08 — neither point tracks the framework change, it's noise from whatever else shares that origin
...
- Root cause: `fetchSite()` in `scripts/detect-cms.ts` had no fallback to plain HTTP when HTTPS failed — `dns-check.mjs` already had this exact fallback, `detect-cms.ts` never did. Reproduced directly: `corsfix.com` over HTTPS throws `SSL alert number 49: access_denied` — a TLS ClientHello-level rejection (WAF/bot-mitigation fingerprinting Node's TLS stack, not an HTTP-layer block — changing headers or User-Agent doesn't help), while plain HTTP succeeds with a 301 to the HTTPS version
- Fixed by extracting the fetch logic into `attemptFetch()` and wrapping it in `fetchSite()`, which retries over `http://` when an `https://` attempt throws, mirroring `dns-check.mjs`'s existing pattern
- Verified against 4 of the 30 affected sites: 3 now fetch successfully over the HTTP fallback (`buyelica.ru` → serves over plain HTTP only; `planx.site` → redirects to `forsale.dynadot.com`, a domain-marketplace parking page). The 4th, `www.mordecai-law.com`, correctly still fails — its HTTP response is itself a 308 redirect back to the same TLS-blocked HTTPS endpoint, so there's genuinely no way to reach real content without a full browser TLS stack
- **Follow-up correction**: `corsfix.com`'s plain-HTTP fallback redirects to an unrelated `tescomobile.com` page — confirmed via `dig`/`curl` that this is a genuine response from corsfix.com's own Cloudflare zone (not ISP hijacking), and separately confirmed in Safari that the real HTTPS site works fine. So the HTTP-fallback body is **not** reliable evidence of the real site's content — a site's plain-HTTP and HTTPS endpoints can serve entirely unrelated things. Fixed by adding a `viaHttpFallback` flag: `processSite()` now short-circuits sites reached only via the fallback to `cms: "Blocked"` (reusing the existing "reachable but unverifiable" bucket) instead of fingerprinting the fallback body as real content
- Found a third domain-broker brand in the process: added `makeParkedDetector("Dynadot", /forsale\.dynadot\.com/i, ...)` following the same pattern as GoDaddy/Sedo/Dan.com/Afternic/Aftermarket
- Net effect: a chunk of the "ambiguous, unhandled" dns-check backlog wasn't actually ambiguous — it was our own scanner giving up too early. Re-running `pnpm detect` should reclassify most of the 30 `alive` sites as either confirmed-not-Astro (movable to a removal PR), a real CMS/parked result, or (for TLS-blocked sites like corsfix.com) an honest `Blocked` instead of perpetual `Error`. The remaining `dead-server` (41) and `broken` (7) buckets are still genuinely unhandled and worth a policy once fresh numbers come in post-fix
- Verified the fix's ground truth using the Safari (Technology Preview) MCP: real WebKit isn't blocked by the WAF's TLS fingerprint check, so it can load sites our Node scanner can't. Confirmed corsfix.com is a genuine live Astro v5.18.1 site (`meta[name=generator]`, 6 `<astro-island>` elements, 13 `/_astro/` asset paths, read via `evaluate_javascript`) and that its HTTP-only fallback really does land on an unrelated tescomobile.com page from the same infra. Spot-checked 7 more sites from the 121-site `Blocked` bucket (77 new TLS-fallback cases, 44 old bot-challenge cases) this way: some (`talale.it`, `agileit.com`, `bachitter.dev`) load completely fine in a real browser and are only blocked for the scanner's fetch signature; others (`herisdia.me`, `alexrybin.tech`) have genuine cert/TLS problems for everyone; `alternateoss.com` looks parked/monetized via a JS ad-network redirect chain. Enough signal to justify scripting a full Safari-MCP sweep of the `Blocked` bucket later — not run yet, deferred to a future session. One gotcha: `navigate_to_url` calls must run strictly sequentially — firing two before the first resolves races on the shared tab and returns a stale page
